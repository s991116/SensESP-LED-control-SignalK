...
void setup() {
  ...
  int listenDelay = 200;
  auto* sk_state = new SKValueListener<bool>("electrical.switches.cabinLights.state", listenDelay);
  sk_state->connect_to(stateIO);

  auto* sk_nightMode = new SKValueListener<bool>("electrical.switches.nightMode", listenDelay);
  sk_nightMode->connect_to(nightModeIO);

  auto* sk_level = new SKValueListener<int>("electrical.switches.cabinLights.level", listenDelay);
